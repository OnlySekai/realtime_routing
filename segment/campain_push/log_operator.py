...
have_data = (
    lambda event: event.get("msisdn")
    and event.get("transaction_id")
    and event.get("event_time")
    and event.get("request_content")
    and event.get("application_code")
)
...
def join_sub_info(event):
    ...
    event.subs_info = {
        k: subs_info.get(k)
        for k in ("pushTimes", "start_subs")
        if subs_info.get(k) is not None
    }
    return event
...
def condition_first_time(event):
    event_time = datetime.fromtimestamp(int(event.eventTime) / 1000).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
...
flow_4_condition = (
    ...
    and event.subs_info.pushTimes == 0
)
# check mua goi pre

flow_2356_condition = lambda event: event.subs_info.pushTimes != 0
